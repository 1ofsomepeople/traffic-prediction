...
class VGRNN(nn.Module):
    ...
    def forward(self, x, edge_idx_list, adj_orig_dense_list, hidden_in=None):
        ...
        for t in range(x.size(0)):
            ...
            # Encoder
            enc_t = self.enc(
                torch.cat([phi_x_t, h[-1]], 1).to(self.device), edge_idx_list[t]) # -1 代表最后一层
            enc_mu_t = self.enc_mu(enc_t, edge_idx_list[t])
            enc_logvar_t = self.enc_logvar(enc_t, edge_idx_list[t])
            # Prior
